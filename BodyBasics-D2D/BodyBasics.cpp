...
void update(Speaker &s, float &x, float &y, float &z, const volume &L_c, const float &x_c, const float &y_c, const float &z_c) {

	//std::cout << "Speaker " << s.getID() << ":\nx: " << s.getX() << ", y: " << s.getY() << ", z: " << s.getZ() << "\n";
	
	float r_0 = sqrt(pow((x - s.getX()), 2) + pow((y - s.getY()), 2) + pow((z - s.getZ()), 2));
	float r = sqrt(pow((s.getX() - x_c), 2) + pow((s.getY() - y_c), 2) + pow((s.getZ() - z_c), 2));
	//std::cout << "r_0: " << r_0 << "\nr: " << r << "\n";
...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    while (WM_QUIT != msg.message)
    {
		...
		for (unsigned int i = 0; i < speakers.size(); ++i) {
			update(speakers[i], x_position, y_position, z_position, L_c, x_c, y_c, z_c);
		}
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
    if (m_hWnd)
    {
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
						...
						if (SUCCEEDED(hr))
						{
							...
							x_position = joints[JointType_Head].Position.X;
							y_position = joints[JointType_Head].Position.Y;
							z_position = joints[JointType_Head].Position.Z;

							//Create a JSON object. 
							json::value obj;
							obj[L"x_position"] = json::value::number(x_position);
							obj[L"y_position"] = json::value::number(y_position);
							obj[L"z_position"] = json::value::number(z_position);

							std::cout << "position_x: " << x_position;
        ...
        StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L"Xposition: %1.3f YPosition: %1.3f Zposition: %1.3f", x_position, y_position, z_position);
